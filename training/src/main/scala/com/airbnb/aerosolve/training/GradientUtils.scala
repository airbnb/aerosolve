...
object GradientUtils {
  ...
  def sparseBoost(gradients : Map[Feature, GradientContainer],
                  weightVector : java.util.Map[Feature, FloatVector],
                  dim :Int,
                  lambda : Double) = {
    ...
    gradients.foreach(kv => {
      val (key, gradient) = kv
      val weight = weightVector.get(key)
      if (weight != null) {
  ...
  def rprop(gradients : Map[Feature, GradientContainer],
            prevGradients : Map[Feature, GradientContainer],
            step : scala.collection.mutable.HashMap[Feature, FloatVector],
            weightVector : java.util.Map[Feature, FloatVector],
            dim :Int,
            lambda : Double,
            deltaMax : Float = 1.0f) = {
  ...
  def gradientDescent(gradients : Map[Feature, GradientContainer],
                      weightVector : java.util.Map[Feature, FloatVector],
                      dim :Int,
                      learningRate: Double,
                      lambda : Double) = {
