...
class TrainingUtilsTest {
  ...
  def testFeatureStatistics(): Unit = {
    val examples = TrainingTestHelper.makeSimpleClassificationExamples(registry)._1
    ...
    try {
      ...
      val statsX = stats.get(registry.feature("loc", "x")).get
      ...
      val statsY = stats.get(registry.feature("loc", "y")).get
      ...
      val statsBias = stats.get(registry.feature("BIAS", "B")).get
      ...
      val statsNeg = stats.get(registry.feature("NEG", "T")).get
      ...
      val dictionary = TrainingUtils.createStringDictionaryFromFeatureStatistics(statsArr,
                                                                   Set(registry.family("$rank")))
      assertEquals(4, dictionary.getDictionary.getEntryCount)
      val ex = TrainingTestHelper.makeExample(2.0, 1.0, 2, registry)
      log.info(ex.toString)
      val vec = dictionary.makeVectorFromSparseFloats(ex.only)
      ...
      // Neg is missing
      assertEquals(0.0, sorted.head, 0.2)
      // Y
