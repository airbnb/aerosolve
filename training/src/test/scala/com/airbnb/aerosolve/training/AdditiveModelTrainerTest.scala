...
class AdditiveModelTrainerTest {
  ...
  def testAdditiveModelTrainer(loss : String, dropout : Double, extraArgs : String, exampleFunc: String = "poly") = {
    // test additive model with only Spline functions
    ...
    try {
      ...
      testModel(model, examples, label, numPos)
    } finally {
      sc.stop
      sc = null
  ...
  def testModel(model: AdditiveModel, examples: ArrayBuffer[Example], label: ArrayBuffer[Double], numPos: Int): Unit = {
