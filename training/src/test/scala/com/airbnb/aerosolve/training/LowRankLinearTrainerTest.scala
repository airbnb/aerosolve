...
class LowRankLinearTrainerTest {
  ...
  def testLowRankLinear(lambda : Double,
    ...
    try {
      ...
      val model = LowRankLinearTrainer.train(sc, input, config, "model_config", registry)

      val featureWeightVector = model.featureWeightVector.asScala
      for (wv <- featureWeightVector) {
        log.info(wv.toString())
      }

      val labelWeightVector = model.labelWeightVector.asScala
