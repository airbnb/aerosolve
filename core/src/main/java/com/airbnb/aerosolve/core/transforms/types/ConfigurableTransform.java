...
public abstract class ConfigurableTransform<T extends ConfigurableTransform>
  ...
  protected void validate() {
    ...
    if (numViolations > 0) {
      throw new ConstraintViolationException(
          String.format("Transformer failed validation with %d violations", numViolations),
          violations);
    }
  }
  ...
  protected static double doubleFromConfig(Config config, String key, String field) {
    return doubleFromConfig(config, key, field, null);
  }

  protected static Double doubleFromConfig(Config config, String key, String field,
                                         Double defaultValue) {
    String path = key + field;
    if (defaultValue == null || config.hasPath(path)) {
      return config.getDouble(path);
    }
    return defaultValue;
  }

  protected static int intFromConfig(Config config, String key, String field) {
    return intFromConfig(config, key, field, null);
  }

  protected static Integer intFromConfig(Config config, String key, String field, Integer defaultValue) {
    String fullPath = key + field;
    if (defaultValue == null || config.hasPath(fullPath)) {
