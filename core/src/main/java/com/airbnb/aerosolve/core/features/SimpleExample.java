 ...
 *
 */
public class SimpleExample implements Example, Serializable {
  ...
  public SimpleExample(FeatureRegistry registry) {
    this.context = new BasicMultiFamilyVector(registry);
  ...
  public SimpleExample(ThriftExample example, FeatureRegistry registry) {
    ...
    if (example.getContext() != null) {
      this.context = new BasicMultiFamilyVector(example.getContext(), registry);
    } else {
      this.context = new BasicMultiFamilyVector(registry);
    }
    ...
    if (example.getExample() != null) {
      for (ThriftFeatureVector vector : example.getExample()) {
        vectors.add(new BasicMultiFamilyVector(vector, registry));
      }
    }
  }
  ...
  public MultiFamilyVector createVector() {
    return addToExample(new BasicMultiFamilyVector(registry));
  }
  ...
  @Override
  public Example transform(Transformer transformer, AbstractModel model) {
