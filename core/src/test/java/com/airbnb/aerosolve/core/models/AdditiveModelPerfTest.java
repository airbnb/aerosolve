...
public class AdditiveModelPerfTest {
  ...
  public void testPerformance() throws Exception {
    ...
    BasicMultiFamilyVector fastVector = (BasicMultiFamilyVector) Util.decodeFeatureVector(vectorStr,
                                                                                        registry);
  ...
  private Example newExample(BasicMultiFamilyVector fastVector, List<Feature> features) {
    Example example = new SimpleExample(registry);
    for (int i = 0; i < 1000; i++) {
      fastVector = new BasicMultiFamilyVector(fastVector);
